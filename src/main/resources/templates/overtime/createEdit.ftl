...
<@standerPage.Layout>
    ...
    <div>
        <button id="btnSaveOvertime" class="btn  <#if mode == "0"> btn-primary  <#else>  btn-success </#if>">
            <#if mode == "0"> Save<#else>Edit</#if>
        </button>
    </div>
    <input id="mode" value="${mode}" hidden />
    <form id="overtimeForm" class="row">
    <input id="overtimeId" name="overtimeId" class="hidden" value="<#if mode == "1">${overtime.overtimeId}</#if>"  />
        ...
        <div class="form-group col-6">
            ...
            <input type="time" class="form-control" id="startTime" name="startTime"
                   <#if mode == "1">value="${overtime.startTime}"</#if>/>
        </div>
        <div class="form-group col-6">
            <label for="id">End Time</label>
            <input type="time" class="form-control" id="endTime" name="endTime"
                   <#if mode == "1">value="${overtime.endTime}"</#if>/>
        </div>
    </form>
